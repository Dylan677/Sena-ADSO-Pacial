...
class UserModel {
  ...
  async loginUser(req, res) {
    try {
      ...
      const sqlQuery = `
        SELECT u.User_id, u.User_user, u.User_email, u.User_password,
               r.Roles_name AS role, u.User_status_fk
        FROM users u
        LEFT JOIN roles r ON u.Roles_fk = r.Roles_id
        WHERE u.User_user = ? OR u.User_email = ?
        LIMIT 1
      `;
      const [result] = await connect.query(sqlQuery, [user, user]);
      if (result.length === 0) {
        return res.status(400).json({ error: "Usuario no encontrado" });
      }
      const foundUser = result[0];
      const validPassword = await comparePassword(password, foundUser.User_password);
      if (!validPassword) {
        return res.status(400).json({ error: "Contraseña incorrecta" });
      }
      res.status(200).json({
        id: foundUser.User_id,
        username: foundUser.User_user,
        email: foundUser.User_email,
        role: foundUser.role ?? 'Sin rol',
        status: foundUser.User_status_fk,
      });
    } catch (error) {
